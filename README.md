# LiveSub — 直播实时语音字幕（本地 Qwen3-ASR）

将麦克风语音**实时**转成字幕，显示在透明置顶窗口中，供 OBS 等直播软件窗口捕获。
纯本地运行（不依赖任何云端 API），基于 **Qwen3-ASR-1.7B** + **llama.cpp** 推理。
...
| 语言 | C++20，CMake + Ninja + MinGW-w64（llvm-mingw） |
...
- 🖥️ **系统托盘**：右下角状态图标（蓝=就绪 / 绿=识别中 / 红=出错），右键菜单：设置 / 显示隐藏字幕 / 记录讲话稿 / 退出
- 📝 **讲话稿记录**：托盘一键开始/结束，定稿内容实时写入桌面 `讲话记录_年月日_时分秒.txt`
- 🪟 **OBS 集成**：透明置顶字幕窗（窗口捕获，WGC 可保留透明度）；另支持 SRT/文本输出
- 🌐 **多语言**：中/英/日/韩自动检测
- ⚙️ **可配置**：中心点定位、字号/颜色/背景透明度（0-100%）、VAD 阈值、更新步长等
...
13. **新段首窗太短会碎片闪现**（0.5s 音频识别出"文。"之类）→ 新段首窗等满 1.5s 再识别，首版字幕直接较完整。
...
| 平台 | 单窗口计算（6s 窗口） |
|---|---|
| RX 7800 XT（Vulkan，Q8_0） | ~0.25s（enc ~35ms + dec ~100ms） |
| Xeon E5-2696 v3（36 线程 CPU） | ~1.2-1.8s |

显存占用约 3GB。端到端延迟（GPU）：VAD 触发后 1.5s 出首版字幕，之后每 0.8s 平滑更新，停顿 0.8s 定稿。
